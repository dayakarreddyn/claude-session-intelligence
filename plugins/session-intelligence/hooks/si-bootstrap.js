...
// ─── Memory index generator (JIT loading discipline) ────────────────────────
//
// Anthropic's memory tool spec advocates just-in-time retrieval: don't load
// memory upfront, fetch on demand. SI's user-level CLAUDE.md auto-memory
// block currently inlines `MEMORY.md` (capped at 200 lines) into every
// session's context — bytes paid every cold start, regardless of whether
// memory is relevant to the current task.
//
// This generator maintains a tiny `MEMORY.index.md` next to `MEMORY.md`:
// title + one-line description per entry, no body, capped at 50 entries.
// Users opt into JIT loading by switching their auto-memory block to
// reference the index instead. SI keeps the index in sync; the agent
// fetches the full body via Read on demand when the index hints at
// something useful.
//
// Idempotent — re-running on an unchanged MEMORY.md is a no-op (we hash
// the source and compare). Failures are logged at debug and never block
// bootstrap.
function regenerateMemoryIndex(cwd) {
  if (!cwd) return false;
  const projectDir = resolveProjectDir(cwd);
  if (!projectDir) return false;
  const memoryDir = path.join(projectDir, 'memory');
  const src = path.join(memoryDir, 'MEMORY.md');
  const dst = path.join(memoryDir, 'MEMORY.index.md');
  let txt;
  try { txt = fs.readFileSync(src, 'utf8'); }
  catch (err) {
    intelLog('bootstrap', 'debug', 'memory index — source missing', {
      src, err: err && err.message,
    });
    return false;
  }

  // Hash source + cap to detect no-ops cheaply.
  const crypto = require('crypto');
  const cap = 50;
  const sha = crypto.createHash('sha1').update(txt).update(`cap=${cap}`).digest('hex').slice(0, 12);
  const sentinel = `<!-- si:memory-index sha=${sha} cap=${cap} -->`;
  try {
    const existing = fs.readFileSync(dst, 'utf8');
    if (existing.includes(sentinel)) return false; // already in sync
  } catch { /* dst missing — proceed */ }

  // Parse MEMORY.md bullet entries. Format we honour:
  //   - [Title](slug.md) — one-line description
  //   - [Title](slug.md): description
  // Anything else passes through verbatim. We only keep the first `cap`
  // entries — older items fall off implicitly. The order is preserved
  // because MEMORY.md is append-style.
  const entries = [];
  ...
  const lines = [
    `<!-- si:memory-index — auto-generated by si-bootstrap. Do not edit manually; updates land on next SessionStart. -->`,
    sentinel,
    `# Memory index (${entries.length}/${cap})`,
    '',
    'One line per memory file. Pull the full body via `Read` on demand — the agent should treat this as a *catalog*, not the memory itself.',
    '',
    ...entries,
    '',
  ];
  ...
  try {
    ...
    intelLog('bootstrap', 'info', 'memory index regenerated', {
      dst, entries: entries.length, sha,
    });
    return true;
  } catch (err) {
    intelLog('bootstrap', errLogLevel(err), 'memory index write failed', {
      dst, err: err && err.message,
    });
    return false;
  }
}
...
// ─── Hygiene nudge ───────────────────────────────────────────────────────────
//
// Mirrors Anthropic's auxiliary memory-tool prompt:
//   "Note: when editing your memory folder, always try to keep its content
//   up-to-date, coherent and organized. You can rename or delete files
//   that are no longer relevant. Do not create new files unless necessary."
//
// We surface this as additionalContext (not stderr) so it lands as a
// model-visible directive rather than an operator banner. Once-per-day
// rate-limited via state — repeated SessionStarts in the same day stay
// silent so it doesn't compete with the rest of bootstrap's signals.
function buildMemoryHygieneContext(state) {
  const today = new Date().toISOString().slice(0, 10);
  if (state.memoryHygieneShown === today) return '';
  state.memoryHygieneShown = today;
  return [
    'Memory hygiene reminder (once per day):',
    'When editing your memory folder, keep it up-to-date, coherent, and organized.',
    'Prefer renaming/extending an existing file over creating a new one.',
    'Delete entries that are no longer relevant. Update the index in place.',
  ].join('\n');
}
...
function main() {
  ...
  try {
    ...
    // Memory index regeneration — keeps MEMORY.index.md in sync with
    // MEMORY.md so the JIT loading discipline has a tiny catalog to fetch.
    // Idempotent; the sentinel-hash short-circuits when MEMORY.md is
    // unchanged. Runs inside the lock since other bootstrap state writes
    // do — no conflict, just shares the existing critical section.
    let didRegenIndex = false;
    try { didRegenIndex = regenerateMemoryIndex(cwd); }
    catch (err) {
      intelLog('bootstrap', errLogLevel(err), 'memory index regen failed', {
        err: err && err.message,
      });
    }

    // Memory hygiene nudge — daily-rate-limited, mirrors Anthropic's
    // memory-tool auxiliary prompt. Stored on state so subsequent same-
    // day SessionStarts stay silent.
    const hygieneContext = buildMemoryHygieneContext(state);

    if (didSeedConfig || didWire || didSeedCtx || didAutoFill || didInject
        || didShowTips || didRegenIndex || hygieneContext) {
      saveState(state);
    }
    // Stash hygiene context on the closure for emit below. Building it
    // inside the lock keeps the once-per-day check race-free across
    // concurrent session opens.
    main.__hygieneContext = hygieneContext;
  } finally {
  ...
  const hygiene = main.__hygieneContext || '';
  ...
  emitAdditionalContext([continuation, nexus, hygiene], systemMessage);
